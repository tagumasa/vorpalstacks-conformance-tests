...
resource "aws_dynamodb_table" "with_stream" {
  name         = "tf-test-with-stream"
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "id"
...
data "aws_dynamodb_table" "basic" {
  name = aws_dynamodb_table.basic.name
}

data "aws_dynamodb_table" "with_gsi" {
  name = aws_dynamodb_table.with_gsi.name
}

data "aws_dynamodb_table" "with_stream" {
  name = aws_dynamodb_table.with_stream.name
}
