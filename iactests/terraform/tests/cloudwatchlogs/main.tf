...
resource "aws_cloudwatch_log_group" "basic" {
  name              = "/terraform/test/basic"
  retention_in_days = 30
}

resource "aws_cloudwatch_log_group" "custom_retention" {
  name              = "/terraform/test/custom-retention"
  retention_in_days = 14
}

resource "aws_cloudwatch_log_stream" "test_stream" {
  name           = "tf-test-stream"
  log_group_name = aws_cloudwatch_log_group.basic.name
}

data "aws_cloudwatch_log_group" "basic" {
  name = aws_cloudwatch_log_group.basic.name
}

data "aws_cloudwatch_log_group" "custom_retention" {
  name = aws_cloudwatch_log_group.custom_retention.name
}
