...
resource "aws_kinesis_stream" "basic" {
  name        = "tf-test-basic-stream"
  shard_count = 1
}

resource "aws_kinesis_stream" "enhanced" {
  name        = "tf-test-enhanced-stream"
  shard_count = 2
...
data "aws_kinesis_stream" "basic" {
  name = aws_kinesis_stream.basic.name
}

data "aws_kinesis_stream" "enhanced" {
  name = aws_kinesis_stream.enhanced.name
}
